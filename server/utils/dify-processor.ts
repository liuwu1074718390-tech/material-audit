...
// 日志函数
const logFile = '/Users/liuwu/Desktop/材价审计/dify-debug.log'
...
async function processGroup(
  ...
  city?: string,
  date?: string,
  deduplicatedMaterialToIdMap?: Map<ExcelRowData, string>
...
export async function processAuditTask(
  ...
  const duplicateMaterialIdsMap = new Map<string, string[]>() // 记录去重后材料ID对应的所有原始材料ID列表
  ...
  for (const material of filteredMaterials) {
    ...
    if (!seenUniqueKeys.has(uniqueKey)) {
      ...
      duplicateMaterialIdsMap.set(materialId, [materialId])
  ...
  if (duplicateCount > 0) {
    log(`[Dify] 去重前: ${filteredMaterials.length} 条，去重后: ${deduplicatedMaterials.length} 条，去除重复: ${duplicateCount} 条`)
    log(`[Dify] 去重映射关系: ${Array.from(duplicateMaterialIdsMap.entries()).map(([id, ids]) => `${id}->[${ids.join(',')}]`).join('; ')}`)
  }
  ...
  // 保存去重前的原始材料数量（用于进度计算）
  // 注意：结果会展开到所有原始材料，所以totalMaterials应该使用去重前的数量
  const originalFilteredCount = filteredMaterials.length
  const originalFilteredMaterials = [...filteredMaterials] // 保存去重前的原始材料列表
  ...
  for (let i = 0; i < filteredMaterials.length; i += 10) {
    groups.push(filteredMaterials.slice(i, i + 10))
  }
  ...
  try {
    ...
    for (let i = 0; i < groups.length; i += maxConcurrent) {
      ...
      log(`[Dify] 开始处理第 ${i / maxConcurrent + 1} 批，共 ${batch.length} 组`)
      
      // 并发处理这一批
      const promises = batch.map((group, idx) => {
        log(`[Dify] 处理第 ${i + idx + 1} 组，材料数: ${group.length}`)
        log(`[Dify] 使用的 materialMap 大小: ${task.materialMap.size}`)
        // 传递去重后的材料到ID的映射，确保ID唯一
        return processGroup(taskId, group, task.materialMap, cityCode, dateRange, deduplicatedMaterialToIdMap)
      })
      
      const batchResults = await Promise.all(promises)
      
      log(`[Dify] 第 ${i / maxConcurrent + 1} 批处理完成`)
      
      // 添加结果到任务
      let totalResultsAdded = 0
      for (const results of batchResults) {
        log(`[Dify] 准备添加 ${results.length} 条结果`)
        if (results.length > 0) {
          // 将去重后的结果复制到所有重复的原始材料
          const expandedResults: AuditResultData[] = []
          for (const result of results) {
            const duplicateIds = duplicateMaterialIdsMap.get(result.ID)
            if (duplicateIds && duplicateIds.length > 0) {
              // 这个结果对应一个或多个原始材料，为每个原始材料创建一份结果
              log(`[Dify] 结果 ${result.ID} 对应 ${duplicateIds.length} 个原始材料，展开结果`)
              ...
              // 如果找不到映射关系，尝试直接使用result（兼容旧逻辑）
              log(`[Dify] ⚠️ 警告: 结果 ${result.ID} 在 duplicateMaterialIdsMap 中找不到映射关系，直接添加`)
              expandedResults.push(result)
            }
          }
          
          const resultPreview = expandedResults.slice(0, 3).map(r => ({
            ID: r.ID,
            名称: r.名称,
            状态: r.状态,
            推荐价格范围: r.推荐价格范围
          }))
          log(`[Dify] 结果详情（前3条，已展开重复）: ${JSON.stringify(resultPreview, null, 2)}`)
          log(`[Dify] 去重后结果数: ${results.length}，展开后结果数: ${expandedResults.length}`)
          
          log(`[Dify] 最终展开后结果数: ${expandedResults.length}，应该等于筛选后的原始材料数: ${originalFilteredCount}`)
          await taskManager.addResults(taskId, expandedResults)
          totalResultsAdded += expandedResults.length
          log(`[Dify] 已添加 ${expandedResults.length} 条结果（展开后），累计: ${totalResultsAdded}`)
        } else {
          log(`[Dify] ⚠️ 警告: 这一批的结果数组为空，跳过保存`)
        }
      }
      log(`[Dify] 本批次共添加 ${totalResultsAdded} 条结果`)
        ...
        if (!savedMaterialIds.has(materialId)) {
          ...
          if (deduplicatedId) {
            ...
            if (deduplicatedResult) {
              log(`[Dify] 为遗漏的材料 ${materialId} 创建结果（使用去重后材料 ${deduplicatedId} 的结果）`)
              missingResults.push({
                ...material,
                ID: materialId,
            ...
            log(`[Dify] ⚠️ 警告: 材料 ${materialId} 找不到对应的去重后材料ID`)
          }
        }
      }
    ...
    log(`[Dify] 任务 ${taskId} 完成，共处理 ${finalTask?.results.length || 0} 条结果，应该等于原始材料数: ${originalFilteredCount}`)
  } catch (error) {
