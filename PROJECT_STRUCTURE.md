...
```
材价审计/
├── .nuxt/                    # Nuxt构建输出目录（自动生成）
├── .output/                  # 生产构建输出目录
├── assets/                   # 静态资源
│   └── styles/
│       └── main.scss        # 全局样式文件
├── components/              # Vue组件
│   ├── UploadDialog.vue    # 文件上传弹窗组件
│   └── AuditResultTable.vue # 审计结果表格组件
├── composables/             # 组合式函数
│   ├── useExcelParser.ts   # Excel解析工具
│   └── useDifyApi.ts       # Dify API调用工具
├── docs/                    # 文档目录
│   ├── API.md              # API接口文档
│   ├── USAGE.md            # 使用说明
│   ├── DEPLOYMENT.md       # 部署文档
...
│   ├── api/                # API路由
│   │   ├── dify/
│   │   │   └── audit.post.ts      # Dify审计API
│   │   └── audit-records.get.ts   # 查询审计记录API
│   ├── utils/              # 服务端工具
│   │   └── database.ts     # 数据库工具类
