  ...
  async createTask(materials: ExcelRowData[], projectName: string): Promise<string> {
    try {
      ...
      if (materialInserts.length > 0) {
        try {
          const placeholders = materialInserts.map(() => '(?, ?, ?)').join(', ')
          const values = materialInserts.flat()
          await query(
          ...
          console.log(`[TaskManager] 材料数据已保存到数据库 (${materialInserts.length} 条)`)
        } catch (dbError: any) {
  ...
  async addResults(taskId: string, results: AuditResultData[]): Promise<void> {
    ...
    try {
      ...
      await transaction(async (connection) => {
        ...
        console.log(`[TaskManager] 准备插入 ${inserts.length} 条结果，第一条的 task_id: ${inserts[0][0]}, material_id: ${inserts[0][1]}`)
        ...
        console.log(`[TaskManager] 插入结果完成，影响行数:`, insertResult[0])
