  ...
  <div class="container">
    ...
    >
      <div v-if="selectedResults.length === 0" style="text-align: center; color: #909399; padding: 20px;">
        <p>暂无结果数据</p>
        <p style="font-size: 12px; margin-top: 10px;">结果数量: {{ selectedResults.length }}</p>
        <p style="font-size: 12px;">请检查浏览器控制台和服务器日志</p>
      </div>
      <AuditResultTable 
        v-else
...
// 获取完整任务列表（只在初始加载或手动刷新时调用）
const fetchTasks = async () => {
  loading.value = true
  try {
    const data = await $fetch('/api/dify/tasks')
    const newTasks = (data as any[]) || []
    tasks.value = newTasks
    
    // 启动或停止每个任务的局部轮询
    updateTaskPolling(newTasks)
  } catch (err: any) {
...
const handleView = async (task: any) => {
  try {
    ...
    selectedResults.value = info.results || []
    selectedTaskName.value = info.projectName || task.projectName
    resultDialogVisible.value = true
    
    if (selectedResults.value.length === 0) {
      console.warn('[Frontend] 警告: 结果数组为空')
      console.warn('[Frontend] API 返回的完整数据:', info)
      ElMessage.warning(`该任务没有结果数据（数据库中有 ${info.resultCount || 0} 条记录）`)
    }
  } catch (err: any) {
...
const handleDelete = async (task: any) => {
  try {
    ...
    await $fetch(`/api/dify/task/${task.taskId}`, { method: 'DELETE' })
